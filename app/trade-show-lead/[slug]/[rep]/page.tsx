...
import { Upload, CheckCircle, Loader2, Calendar, MapPin, User } from "lucide-react"
...
export default function TradeshowRepLeadForm() {
  const params = useParams()
  const router = useRouter()
  const slug = params.slug as string
  const repCode = params.rep as string
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [tradeshowRes, repRes] = await Promise.all([
          fetch(`/api/tradeshows/by-slug/${slug}`),
          fetch(`/api/reps/by-code/${repCode}`)
        ])

        if (tradeshowRes.ok) {
          const tradeshowData = await tradeshowRes.json()
          setTradeshow(tradeshowData)
        }
        ...
        if (tradeshowRes.ok) {
          const tradeshowData = await tradeshowRes.json()
          track("trade_show_rep_form_view", {
            tradeshow: tradeshowData.name,
            rep: repCode
          })
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      track("trade_show_rep_lead_submission", {
        tradeshow: tradeshow.name,
        rep: rep?.name || repCode,
      ...
      formDataToSend.append("repCode", repCode)
      if (formData.badgePhoto) {
